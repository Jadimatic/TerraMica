...
{
    internal class LaserLanceCannonProjectile : ModProjectile
    {
        ...
        public override void AI()
        {
            ...
            if (Main.myPlayer == Projectile.owner)
            {
                ...
                if (ProjDelay <= 0)
                {
                    Projectile.NewProjectile(Projectile.GetSource_FromThis(null), Projectile.Center, Projectile.velocity * -5f, ModContent.ProjectileType<LaserLanceLaser>(), ProjDamage, 1f, Main.myPlayer, -1, -1);
                    SoundEngine.PlaySound(Main.rand.NextBool() ? SoundID.Item157 : SoundID.Item158, Projectile.position);
                    ProjDelay = (int)MathHelper.Lerp(15, 2, MathHelper.Min(1, owner.velocity.Length() / 5));
                }
            }
